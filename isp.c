...
unsigned char ispTransmit32(unsigned long data)
{
        //PORTD &= ~(1<<PD2); // SCK low
        ispTransmit((data>>24)&0xFF);
        ispTransmit((data>>16)&0xFF);
        ispTransmit((data>>8)&0xFF);
        return(ispTransmit(data&0xFF));
}

unsigned char ispTransmit16(unsigned long data)
{
        ispTransmit((data>>8)&0xFF);
        return(ispTransmit(data&0xFF));
}
...
uint8_t ispWrite8EEPROM8(uint8_t address, uint8_t data) {
